...
public class Python_Airflow {
	...
	public static String getDW(String tableName, String txtFileName, String sourceFileIsZip) {
		...
		String rs = "#!/usr/bin/env python3\n"
				...
				+ "ODS_L06_LoadODS >> file_done >> check_rslt1 >> get_rslt_xcoms1 >> condition_rslt_to_go1 >> BACKUP_DW >> TRUNCATE_DW >> DW_L07_LoadDW\n"
				+ "DW_L07_LoadDW >> DW_L08_LoadDW_TMP >> check_rslt2 >> get_rslt_xcoms2 >> condition_rslt_to_go2\n"
	...
	public static String getDM(String tableName, String txtFileName) {
		...
		String rs = "#!/usr/bin/env python3\n"
				...
				+ "SP_PAR=\"'A','','','\"+target_table+\"';\"\n"
				+ "query_start='''{{ dag_run.conf['query_start'] if dag_run.conf else \"exec spGetETLPrecheck '''+SP_PAR+'''\" }}'''\n"
				+ "date_ctrl='''{{ 'post1_post_poc_raw.T_CMMM_CCSYM_DTL_'+dag_run.conf['date_ctrl'] if dag_run.conf else \"post1_post_poc_raw.T_CMMM_CCSYM_DTL\" }}'''\n"
				+ "query_end=\"exec dbo.spSetETLFinish '\" + target_table + \"';\"\n"
				+ "query_fail=\"exec dbo.spDelRunNow '\" + target_table + \"';\"\n"
				+ "\n"
				+ "def get_xcoms(**kwargs):\n"
				...
				+ "DM_T01 = BashOperator(\n"
				+ "	task_id='DM_T01',\n"
				+ "	bash_command=f\"sh {DAG_WORKSPACE}scripts/gssSQLConn.sh --logic-file {PROG_PATH}{target_table}/bin/DM_T01.hql --var ACT_YM={{{{ ti.xcom_pull(task_ids='get_xcoms' , key='YMS') }}}} --var DATE_CTRL={date_ctrl} \",\n"
				+ "	dag=dag\n"
				+ ")\n"
				+ "\n"
				+ "####################################\n"
				...
				+ "BEFORE_C01 >> DM_T01 >> check_rslt1 >> get_rslt_xcoms1 >> condition_rslt_to_go1 >> BACKUP_DM\n"
				+ "BACKUP_DM >> TRUNCATE_DM >> DM_L02_LoadDM >> DM_L03_LoadDM_TMP >> check_rslt2 >> get_rslt_xcoms2\n"
				+ "get_rslt_xcoms2 >> condition_rslt_to_go2 >> [check_fail, end_status]\n"
