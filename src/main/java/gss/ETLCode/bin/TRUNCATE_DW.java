...
public class TRUNCATE_DW {
	...
	public static String getHQL(String partition, Map<String, String> mapProp, String tableName) {
		...
		String rs = "-----------------------------------------------------------------\n"
				...
				+ "set hivevar:DES1_TruncateDW="+mapProp.get("hadoop.raw.dbname")+"."+tableName+";\n"
				+ "set hivevar:FUNC_NAME="+tableName+"_TruncateDW;\n"
				+ "set hivevar:KEY_NAME=return_code;\n"
				+ "set hivevar:LOGIC_NAME="+tableName+";\n"
				+ "set hivevar:TMP1=tmp_"+tableName+"_TruncateDW;\n"
				+ "-----------------------------------------------------------------\n"
				+ "\n"
				+ "\n";
			rs += !StringUtils.isBlank(partition) ? 
				"-- 有Partiton\n"
				+ "ALTER TABLE ${hivevar:DES1_TruncateDW} DROP IF EXISTS PARTITION("+partition+"='${hivevar:ACT_YM}');\n"
				: "-- 無Partiton\n"
				+ "TRUNCATE TABLE ${hivevar:DES1_TruncateDW};\n" ;
			rs += "\n"
				+ "-- verification 總筆數\n"
				...
				+ "from ${hivevar:DES1_TruncateDW} a \n";
			rs += !StringUtils.isBlank(partition) ? 
	...
	public static String getShell(String tableName) {
		...
		String rs = "########################\n"
				...
				+ "declare SRC1_PATH=/opt/gss/pipe-logic-deploy/post/"+tableName+"/bin/TRUNCATE_DW.hql\n"
				+ "\n"
